...
int Print(HWND hwnd,HWND edt,HWND bufedt,HWND bufEdt1,HWND bufEdt2){
	...
	for(int k=1;k<=loop;k++){
		...
		DrawText(prt.hDC,lpszBuf,Len,&txrc,DT_WORDBREAK|DT_EDITCONTROL|DT_LEFT);//10,10+~のところで余白をいじれる
...
UINT CALLBACK PageSetHook(HWND hdlg , UINT uiMsg , WPARAM wp , LPARAM lp) {
	...
	switch(uiMsg) {
		...
		case WM_COMMAND:
			switch(LOWORD(wp)) {
				case IDOK:
					DEVMODE *dlgDate = (DEVMODE *)GlobalLock(psd.hDevMode);
					switch(SendMessage(GetDlgItem(hdlg, IDC_COMBO1) , CB_GETCURSEL , 0 , 0)){
					case 0:
						prt_duplex = simplex_dm;
						dlgDate->dmDuplex = DMDUP_SIMPLEX;
						break;
					case 1:
						prt_duplex = horizontal_dm;
						dlgDate->dmDuplex = DMDUP_HORIZONTAL;
						break;
					case 2:
						prt_duplex = vertical_dm;
						dlgDate->dmDuplex = DMDUP_VERTICAL;
						break;
					}
					GlobalUnlock(psd.hDevMode);
					SerPrint(hdlg);
