...
char szWindowTitle[128] = "ただのテキストエディター";//ウインドウのタイトル
char szTitle[128] = "";
...
const int insz=128;//検索ダイアログ入力も文字数
TCHAR inBuf[insz]="";
TCHAR repBuf[insz]="";
bool tSE=false;
...
//リターン値	1:見つかりました	2:全く見つかりませんでした	3:すべて検索しました
	...
	if(ud=='d'){
		...
		while(Flug){
			...
			if(tS>=end&&sw==true){
				...
				if(textBuf[tS]==inBuf[0]){
					while(i+1){
						if(i==m){i=-1;
							if(textBuf[tS+m]==inBuf[m]){tE=tS+m+1;Flug=0;return 1;}//FOUND
						}
						else if(textBuf[tS+i]!=inBuf[i]){i=-1;}
						else {i++;}
					}
				}
				else i=-1;
		...
		while(Flug){
			...
			if(/*tS>=end*/tS<=end&&sw==true){
				SendMessage(edt,EM_SETSEL,end,end);
				return 2+fs;
			}//NOTFOUND
			else if(/*tS>nLen*/tS<0){
				tS=nLen+1;
				sw=true;
			}
			else{
				...
				if(textBuf[tS]==inBuf[/*0*/m]){
					while(i+1){
						if(i==m){i=-1;
							if(textBuf[/*tS+m*/tS-m]==inBuf[/*m*/0]){tS-=m;tE=tS+m+1;Flug=0;return 1;}//FOUND
						}
						else if(textBuf[/*tS+i*/tS-i]!=inBuf[/*i*/m-i]){i=-1;}
						else {i++;}
					}
				}
				else i=-1;
	...
	if(all){
		...
		switch(FINDTEXT(edt,ud)){
			...
			case 2:
				tSE=false;
				return 2;
			case 3://なぜか121212などを置き換えようとすると最後の1つだけ置換されずにここへ来る
				tSE=false;
				return 3;
		}
	}
...
int Print(HWND hwnd,HWND edt){
	...
	dpi=GetDeviceCaps(prt.hDC,LOGPIXELSX);
	txrc.right=GetDeviceCaps(prt.hDC,HORZRES);
	txrc.bottom=GetDeviceCaps(prt.hDC,VERTRES);
	fonts=lf.lfHeight;
	lf.lfHeight=(fonts/(96/72))*(dpi/96);
	hFont1 = CreateFontIndirect(&lf);

	memset(&di,0,sizeof(DOCINFO));

	di.cbSize=sizeof(DOCINFO);
	di.lpszDocName="TEST";//ドキュメント名
	//di.lpszOutput="TOUT";//出力ファイル名//******書いてはいけない******//
	...
	if(msg==FNDm&& !(frp.Flags & FR_DIALOGTERM)){
		...
		if(msg==FNDm&&(frp.Flags & FR_REPLACE)){//置き換え
			...
			switch(FINDTEXT(edt,ud)){
				...
				case 2:			
					FTF1=false;
					MessageBox(hwnd,"404 NOT FOUND","404ERORR",MB_ICONINFORMATION);
					tSE=false;
					break;
				case 3:
					FTF1=false;
					MessageBox(hwnd,"すべての検索が終わりました","完了！",MB_ICONINFORMATION);
					tSE=false;
					break;
	...
	switch (msg) {
		...
		case WM_CREATE:
			...
			bufEdt1=CreateWindow(
				...
				ES_AUTOVSCROLL |WS_BORDER |ES_NOHIDESEL |
			...
			cf.lpLogFont = &lf;
			cf.Flags = /*CF_EFFECTS// |*/
				CF_SCREENFONTS | CF_INITTOLOGFONTSTRUCT | CF_SELECTSCRIPT ;//Option
			hFont1 = CreateFontIndirect(&lf);
		...
		case WM_COMMAND:
			...
			switch(LOWORD(wp)){
				...
				//---Help---//
				case IDM_HELP:
					MessageBox(hwnd,"詳細はReadMeをお読みください",szWindowTitle,MB_ICONINFORMATION);
					break;
				}
			return 0;
