...
/*	prt.nMinPage = 1;//ページ範囲の最小
	...
	wsprintf(StasBuf,"印刷 - 出力中");
	SendMessage(Stas,SB_SETTEXT,0|0,(LPARAM)StasBuf);
	for(int k=1;k<=loop;k++){
		SetWindowText(bufEdt1,"");
		wsprintf(StasBuf,"印刷 - 出力中　%dページ中%dページ目",loop,k);
		SendMessage(Stas,SB_SETTEXT,0|0,(LPARAM)StasBuf);
		...
		DrawTextW(prt.hDC,Buf,Len,&txrc,DT_WORDBREAK|DT_EDITCONTROL|DT_LEFT);//10,10+~のところで余白をいじれる
...
int SetStasLLen(HWND edt){
	...
	if(S==E)wsprintf(StasBuf,"    行:%d    列:%d",Line+1,E-LLen);
	else wsprintf(StasBuf,"文字数:%d",E-S);
