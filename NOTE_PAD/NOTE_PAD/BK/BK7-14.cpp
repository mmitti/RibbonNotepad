...
int FINDTEXTR2(HWND edt,int ct){
//英数字1文字→英数字1文字の置換OK
	char* rc ="";
		...
		if(Loop>nLen){
			...
			if(textBuf[Loop] == inBuf[0]){
				...
				while(i+1){
					if(i == inLen-1){
						i = -1;
						if(textBuf[Loop + OutLen] == inBuf[OutLen]){
					...
					else if(textBuf[Loop+i] != inBuf[i]){
						textOutBuf[Loop + OutLen] = textBuf[Loop + BufLen];
						Loop++;	
						i = -1;
					}
					else i++;
